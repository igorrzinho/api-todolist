...
export async function createTableTodo() {
  openDb().then((db) => {
    db.exec(
      'CREATE TABLE IF NOT EXISTS todo (id integer PRIMARY KEY, task varchar(255) DEFAULT NULL, id_usuarioFK INT NOT NULL, FOREIGN KEY (id_usuarioFK) REFERENCES usuarios (id));'
    );
  });
}
